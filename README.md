...
- [Installing MongoDB on your Machine](#installing-mongodb-on-your-machine)
  ...
  - [5. Sarah's Updated Instructions for MacOS](#5-sarahs-updated-instructions-for-macos)
...
## 5. Sarah's Updated Instructions for MacOS

1. If you had MongoDB installed previously, uninstall it using these instructions:

   1. `brew services stop mongodb` (this may throw an error if the process is not running)

   2. `brew uninstall mongodb`

2. Use the following commands to install MongoDB using Homebrew:
...
3. If you ever wish to turn your server off, run the command `brew services stop mongodb-community`; turn it back on with `brew services start mongodb-community`

4. You're ready to interface with your MongoDB server! Enter this command in your terminal window: `mongo`
